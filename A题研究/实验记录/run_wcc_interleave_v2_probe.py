"""Pre-register the fixed five-case v2 priority-preservation probe; P2 only."""
import argparse
...
def prepare(out):
    ...
    source_files = [Path(__file__), RESEARCH / "精修求解器/wcc_interleave.py",
        RESEARCH / "精修求解器/wcc_interleave_v2.py",
        RESEARCH / "精修求解器/README_wcc_interleave_v2.md",
        RESEARCH / "精修求解器/tests/test_wcc_interleave.py",
        RESEARCH / "精修求解器/tests/test_wcc_interleave_v2.py"]
    ...
    v1_path = RESEARCH / "精修求解器/runs/wcc_interleave_v1"
    v1_manifest, v1_summary = read(v1_path / "manifest.json"), read(v1_path / "summary.json")
    inputs.update({str(v1_path / "manifest.json"): digest(v1_path / "manifest.json"),
                   str(v1_path / "summary.json"): digest(v1_path / "summary.json")})
    for case in CASES:
        name = "case_" + case
        ...
        prior = next(c for c in v1_manifest["cases"] if c["case"] == name)
        if object_digest(plan) != prior["baseline_plan_sha256"] or record != prior["baseline_record"]:
            raise ValueError("v2 must use the identical frozen v1 baseline plan and record")
    ...
    manifest = {"schema_version": 1, "created_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "max_p2_calls": 45, "max_p3_calls": 0, "planned_p2_calls": sum(len(x["candidates"]) for x in cases),
        "p3_rule": "no P3 calls authorized in v2 probe",
        "v1_summary_path": str(v1_path / "summary.json"),
        "cases": cases}
    if manifest["planned_p2_calls"] > 45:
        raise ValueError("pre-registered P2 call cap exceeded")
...
def run(out):
    ...
    for case in m["cases"]:
        ...
        cases.append(row)
    v1_cases = {c["case"]: c for c in read(m["v1_summary_path"])["cases"]}
    for case in cases:
        prior = v1_cases[case["case"]]
        case["v1_selected_makespan"] = key(prior["selected_record"])[0]
        case["v1_best_new_makespan"] = key(prior["best_new"]["record"])[0]
