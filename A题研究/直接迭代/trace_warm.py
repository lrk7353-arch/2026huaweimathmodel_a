...
def run(case,old,out,budget=8,seconds=120,cores=5):
 if key(old)!=(case,2,cores):raise ValueError('Incumbent case/problem/core mismatch')
 ...
 for rd in range(3):
  ...
  cs,_=generate_trace_candidates(ir,plan,raw,num_cores=cores,max_candidates=8,round_index=rd,seed=17)
  before=len(calls)
  for c in cs:
 ...
 s={'case':case,'problem':2,'num_cores':cores,'before':score(old)[0],'after':score(best)[0],'best_record':best,'calls':calls,'logical_calls':len(calls),'new_calls':sum(not c['record']['cache_hit'] for c in calls),'elapsed_seconds':time.monotonic()-t,'stop_reason':'time_budget' if time.monotonic()>=deadline else 'budget_or_candidates'}
