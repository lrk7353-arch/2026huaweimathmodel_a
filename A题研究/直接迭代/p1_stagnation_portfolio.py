# Experimental derivative of cold_portfolio.py at d260720.
# Its original queues and control flow remain intact; the optional late queue
# is injected once after stagnation. Keep the frozen mature module unchanged.
...
def run(case,problem,cores,variant,out,budget=12,seconds=120,evaluation_timeout=25,evaluation_dir=None):
    ...
    late_considered=False
    while available():
        if problem==1 and not late_considered and len(calls)>=max(6,budget-4):
            late_considered=True
            from p1_late_policy import admit,proposals
            gate=admit(calls)
            stages.append(dict(phase='late_stagnation_gate',**gate))
            if gate['admitted'] and best is not None:
                t=time.monotonic()
                cs,diagnostics=proposals(ir,best,cores,min(deadline,t+15))
                paired.extend(dict(c,paired_source_record=best['record_path']) for c in cs)
                stages.append(dict(phase='late_stagnation_generation',**diagnostics))
        evaluated=False
